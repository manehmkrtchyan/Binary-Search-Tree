...
class BinarySearchTree:
    ...
    def __add__(self, other):
        if not self.root and not other.root:
            return None
        value1 = self.root.value if self.root else 0
        value2 = other.root.value if other.root else 0
        root = Node(value1 + value2)
        root.left_child = (self.root.left_child if self.root else None).__add__(other.root.left_child if other.root else None)    
        root.right_child = (self.root.right_child if self.root else None).__add__(other.root.right_child if other.root else None) 
